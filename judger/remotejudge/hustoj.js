...
const { CompileError, TooFrequentError } = require('../error');
const {
    ...
    STATUS_MEMORY_LIMIT_EXCEEDED, STATUS_JUDGING,
} = require('../status');
...
class HUSTOJ {
    ...
    async login(username, password) {
        ...
        const res = await this.axios.post(this.LOGIN[0], {
    ...
    async submit(pid, code, lang) {
        const res = await this.axios.post(this.SUBMIT[0], {
            [this.SUBTMI[1]]: pid,
        ...
        const url = this.MONIT[0].replace('{uid}', this.state.username).replace('{pid}', pid);
        const r = await this.axios.get(url);
        const rid = this.RID.exec(r.data)[1];
        return {
    ...
    async monit(data, next, end) {
        ...
        const RE = new RegExp(`<tr.*?class="evenrow".*?><td>${data.rid}</td>.*?</td><td>.*?</td><td><font color=".*?">(.*?)</font></td><td>(.*?)<font color="red">kb</font></td><td>(.*?)<font color="red">ms`, 'gmi');
        ...
        while (this.PROCESSING.includes(status)) {
            ...
            [, status, time, memory] = RE.exec(resp.data);
        }
        if (this.CEINFO[0].includes(status)) {
            url = this.CEINFO[1].replace('{rid}', data.rid);
            const resp = await this.axios.get(url);
            const compilerText = decodeURIComponent(this.CEINFO[2].exec(resp.data)
                .replace('\n', '').replace('<br/>', '\n').replace('\n\n', '\n'));
            throw new CompileError(compilerText);
...
class YBT extends HUSTOJ {
    ...
    async supermonit(data, next, end) {
        const url = `/statusx1.php?runidx=${data.rid}`;
        ...
        if (this.CEINFO[0].includes(staText[4])) {
            const url = this.CEINFO[1].replace('{rid}', data.rid);
            const res = await this.axios.get(url);
            const compilerText = decodeURIComponent(this.CEINFO[2].exec(res.data)[1]
                .replace('\n', '').replace('<br/>', '\n').replace('\n\n', '\n'));
            throw new CompileError(compilerText);
        }
    ...
    async supermonit(data, next, end) {
        ...
        if (msg[0] === 'compile error') throw new CompileError(msg.join('\n'));
            ...
            for (let i = 2; i < msg.length - 1; i++) {
                const [, time, memory, score, status] = this.SUPERMONIT[1].exec(msg[i]);
                next({
                    status: STATUS_JUDGING,
                    case: {
                        time_ms: parseInt(time),
        ...
        const [, time, memory, score, status] = this.SUPERMONIT[1].exec(msg[1]);
        end({
            time_ms: parseInt(time),
            memory_kb: parseInt(memory),
            score: parseInt(score),
